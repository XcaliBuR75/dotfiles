  ...
  windows
    ( \s ->
        if M.member w (W.floating s)
          then W.sink w s
          else (W.float w (W.RationalRect 0.15 0.15 0.75 0.75) s)
    )
...
myFocusColor  = "#B48EAD"   -- This variable is imported from Colors.THEME
...
myKeys c =
  ...
  [ ("M-<Return>", addName "Launch terminal"   $ spawn (myTerminal ++ " msg create-window"))
  , ("M-b", addName "Launch web browser"       $ spawn (myBrowser))
  , ("M-M1-h", addName "Launch htop"           $ spawn (myTerminal ++ " -e htop"))
  , ("M-M1-o", addName "Launch sxiv"           $ spawn "sxiv -rt $HOME/Downloads/images")
  , ("M-M1-<Return>", addName "Launch rofi"    $ spawn "rofi -no-lazy-grab -show drun -theme $HOME/.config/rofi/launchers/type-5/style-1.rasi")]
  ...
  , ("M-o", addName "Floating" $ withFocused toggleFloat)]
...
main = do
  ...
  xmproc0 <- spawnPipe ("xmobar -x 0 -A 0 $HOME/.config/xmobar/" ++ colorScheme ++ "-xmobarrc")
  xmproc1 <- spawnPipe ("xmobar -x 1 -A 0 $HOME/.config/xmobar/" ++ colorScheme ++ "-xmobarrc")
  xmproc2 <- spawnPipe ("xmobar -x 2 -A 0 $HOME/.config/xmobar/" ++ colorScheme ++ "-xmobarrc")
